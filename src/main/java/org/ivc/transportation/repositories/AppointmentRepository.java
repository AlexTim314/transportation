...
public interface AppointmentRepository extends JpaRepository<Appointment, Long> {
    ...
    @Query(value = "select appointment.* from appointment, record, claim where " +
            "record.start_date between :date_start and :date_end and "
            + "appointment.record_id = record.id and "
            + "record.claim_id = claim.id and "
            + "appointment.status = :status order by claim.department_id, record.start_date", nativeQuery = true)
